...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    const isLowEnd = isMobile || ((navigator as any).hardwareConcurrency && (navigator as any).hardwareConcurrency <= 4);
  ...
  /* ---- Reactive prop updates (no remount) ---- */
  ...
  if (!webGLSupported) {
    return (
      <div className={`light-pillar-fallback ${className}`} style={{ mixBlendMode } as React.CSSProperties}>
        WebGL not supported
      </div>
    );
  }
