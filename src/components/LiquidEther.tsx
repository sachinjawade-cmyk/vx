...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class WebGLManager {
      props: any; output!: Output; autoDriver!: AutoDriver; lastUserInteraction = performance.now();
  ...
  /* Update options without re-mounting */
  useEffect(() => {
    ...
    Object.assign(sim.options, { mouse_force: mouseForce, cursor_size: cursorSize, isViscous, viscous, iterations_viscous: iterationsViscous, iterations_poisson: iterationsPoisson, dt, BFECC, resolution, isBounce });
    if (resolution !== prevRes) sim.resize();
    const ad = webglRef.current?.autoDriver;
    if (ad) {
      ad.enabled = autoDemo; ad.speed = autoSpeed; ad.resumeDelay = autoResumeDelay;
      ad.rampDurationMs = autoRampDuration * 1000;
      if (ad.mouse) { ad.mouse.autoIntensity = autoIntensity; ad.mouse.takeoverDuration = takeoverDuration; }
    }
